...
#[derive(Debug)]
pub enum CustomProtocolCommand {
...
#[derive(Debug)]
pub struct CustomProtocolHandle {
    pub cmd_tx: Sender<CustomProtocolCommand>,
    pub event_rx: Receiver<CustomProtocolEvent>,
}
...
pub fn make_litep2p() -> (Litep2p, CustomProtocolHandle) {
...
async fn test() {
    ...
    log::info!("my peer id is {:?}", &peer2);

    let listen_address = litep2p2.listen_addresses().next().unwrap().clone();
    // print out the listen address, then add it to second node
    log::info!("my listen address is {:?}", &listen_address);
        ...
        loop {
            tokio::select! {
                event = litep2p1.next_event() => {
                    println!(
                        "litep2p1 received message from {:?}",
                        event
                    );
                }
                event = litep2p2.next_event() => {println!(
                    "litep2p2 received message from {:?}",
                    event
                );}
            }
        }
